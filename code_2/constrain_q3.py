...
def constrain_A(x):
    ...
    x = x[:24]
    x = x.reshape(3, 8)
    return -np.sum(x, axis=1)[0] + supply[0] + add
...
def constrain_B(x):
    ...
    x = x[:24]
    x = x.reshape(3, 8)
    return -np.sum(x, axis=1)[1] + supply[1] + add
...
def constrain_C(x):
    ...
    x = x[:24]
    x = x.reshape(3, 8)
    return -np.sum(x, axis=1)[2] + supply[2] + add


def constrain_add(x):
    return (np.sum(need) - np.sum(supply)) - np.sum(x[24:])


def constrain_add2(x):
    return np.sum(x[24:]) - 30
