...
def compute_multi_linear_fit(
    records,
    predictions,
    ):
    if isinstance(records,list) and isinstance(predictions,list):
        linear_list=[]
        xymin_list=[]
        xymax_list=[]
        ...
        for i in range(len(predictions)):
            rds=records[i]
            pds=predictions[i]
            coeff = np.polyfit(pds, rds, 1)
            linear_fit = coeff[0] * xx + coeff[1]
            linear_list.append(linear_fit)
        return xx,linear_list,xymin,xymax
    elif isinstance(records,dict) and isinstance(predictions,dict):
        linear_list={}
        xymin_list=[]
        xymax_list=[]
